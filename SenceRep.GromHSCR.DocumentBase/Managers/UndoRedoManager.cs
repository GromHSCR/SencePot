...
namespace SenceRep.GromHSCR.DocumentBase.Managers
{
	...
	{
		public static IDocumentManager AppDocumentManager { get; set; }
		...
		/// Stores undo records
		/// </summary>
		private Stack<IUndoRedoRecord> _undoStack;

		/// <summary>
		/// Stores redo records 
		/// </summary>
		private Stack<IUndoRedoRecord> _redoStack;
		...
		/// </summary>
		private volatile UndoRedoManager _thisObject;

		/// <summary>
		/// Maximum items to store in undo redo stack
		/// </summary>
		protected int _maxItems = 1000000;
		...
		{
			_thisObject = this;
			_undoStack = new Stack<IUndoRedoRecord>();
			_redoStack = new Stack<IUndoRedoRecord>();
		}
		...
		/// </summary>
		/// <returns></returns>
		public IUndoRedoManager Instance()
		{
			return _thisObject;
		}

		public static UndoRedoManager GetCurrentUndoRedoManager()
		{
			if (AppDocumentManager == null) return null;
			return AppDocumentManager.GetCurrentUndoRedoManager();
		}
		...
		public void EndTransaction(UndoTransaction tran)
		{
			...
			{
				var t = _undoStack.Peek() as UndoTransaction;
			...
			{
				var t = _redoStack.Peek() as UndoTransaction;
				if (t != null && t.OperationsCount == 0)
				{
					_redoStack.Pop();
				}
		...
		public void Push<T>(UndoRedoOperation<T> undoOperation, T undoData, string description = "")
		{
			...
			Stack<IUndoRedoRecord> stack = null;
			Action eventToFire;
			...
			{
				if (stack.Count != 0)
					((UndoTransaction)stack.Peek()).AddUndoRedoOperation(new UndoRedoRecord<T>(undoOperation, undoData,
						description));
				else
				{
					stack.Push(new UndoTransaction(description));
					((UndoTransaction)stack.Peek()).AddUndoRedoOperation(new UndoRedoRecord<T>(undoOperation, undoData,
						description));
				}
			}
		...
		{
			return _undoStack.ToArray().Select((input) => input.Name ?? "").ToList();
		}
		...
		{
			return _redoStack.ToArray().Select((input) => input.Name ?? "").ToList();
		}

		public Stack<IUndoRedoRecord> RedoStack
		{
			get { return _redoStack; }
			set
			{
				if ((_redoStack = value) != null) return;

				_redoStack = value;
			}
		}

		public Stack<IUndoRedoRecord> UndoStack
		{
			get { return _undoStack; }
			set
			{
				if ((_undoStack = value) != null) return;
